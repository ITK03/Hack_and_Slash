...
### Codex: 2026-08-06 / フィードバック追補

- 案内矢印を一般的な二等辺三角形へ変更し、能力値表の最終セルにも罫線を表示。
- 高速な通常移動でも敵をすり抜けないよう、移動線分全体に接触判定を追加（移動スキル中は対象外）。
- ドロップ待機を0.3秒、吸着開始を1.5m、回収距離を0.2mへ変更。
- 管理者モードに全装備オプションの任意補正編集を追加。
- 1〜45層の敵HP・攻撃力と5層ごとのボス曲線を、想定装備進行に合わせた段階曲線へ変更。
